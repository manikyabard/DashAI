...
class DashTabularModel:
	...
	def create_custom_tabular_model(data, layers, **kwargs):
		...
		model = DashTabularModel(emb_szs, len(data.cont_names), out_sz=data.c, layers=layers, **kwargs)
		return model
	

class DashTabularModel(Module):
    ...
    def forward(self, x_cat:Tensor, x_cont:Tensor) -> Tensor:   
        # print(x_cat.shape, x_cont.shape)
        # print(self.n_emb)
        if self.n_emb != 0:
