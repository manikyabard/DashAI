...
class TabularFeature(BaseFeature):
    def __init__(self, name, baseline_transforms):
        print('=' * 50, 'TabularFeature', '=' * 50, sep='\n')
        super().__init__(
            name,
            baseline_transforms=baseline_transforms,
            input_transforms=None,
    ...
    def visualize(self, attribution, data, contribution_frac) -> FeatureOutput:
        ...
        data = data.squeeze(0)
        if len(attribution.shape) > 1:
            attribution = attribution.sum(dim=1).detach().numpy()
        normalized_attribution = safe_div(
            attribution, np.linalg.norm(attribution, ord=1), default_value=attribution
        )
        modified = [x for x in normalized_attribution.tolist()]
        return FeatureOutput(
            name=self.name,
            base=data,
            modified=modified,
