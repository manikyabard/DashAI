...
class DashTextLearner:
	...
	def create_text_learner(response):
		...
		if response['text']['model']['type'] == "classifier":
			...
			lm = DashTextLearner.create_text_lm_learner_default(data_lm,
																response['text']['model']['language_model']['default'])
			lm.fit_one_cycle(1)  # TODO decide what to do for the training's sake.
			lm.save_encoder('lm_encoder')
			clas = DashTextLearner.create_text_classifier_learner_default(data, response['text']['model']['classifier'][
				'default'])
		...
		if response['text']['model']['type'] == "language_model":
			...
			if response['text']['model']['language_model']['method'] == "default":
				return DashTextLearner.create_text_lm_learner_default(data, response['text']['model']['language_model'][
					'default'])
